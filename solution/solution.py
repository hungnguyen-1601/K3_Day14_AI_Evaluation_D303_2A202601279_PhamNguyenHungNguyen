...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            "Score the answer for each rubric criterion on a 0.0-1.0 scale.\n"
        ...
        try:
            ...
            scores = {
                str(criterion): float(value) for criterion, value in parsed.items()
            }
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(entry_averages) > 1:
            rest_average = sum(entry_averages[1:]) / len(entry_averages[1:])
            positional_bias = entry_averages[0] > rest_average + 0.1
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            result = evaluator.run_full_eval(
                ...
                # No retrieval trace recorded → keep retrieval metrics None.
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        for metric in metrics:
            new_average = _mean([getattr(r, metric) for r in new_results])
            baseline_average = _mean([getattr(r, metric) for r in baseline_results])
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        low_metrics = [name for name, value in scores.items() if value < 0.5]
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                ...
                "supported by the retrieved context before answering"
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Rewrite the system prompt to restate the user question and "
                "require the answer to address it directly"
            )
        if categories.get("incomplete"):
            suggestions.append(
                "Increase retrieval top-k or chunk size so every condition "
                "and exception needed by the expected answer is available"
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Add intent detection and routing so questions map to the "
                "correct policy documents before generation"
            )
        if categories.get("refusal"):
            suggestions.append(
                "Relax over-strict guardrails so in-scope policy questions "
                "are answered instead of refused"
            )

        general_suggestions = [
            "Add few-shot examples showing complete, grounded answers to "
            "improve completeness and faithfulness",
            "Rerank retrieved chunks by query overlap so relevant evidence "
            "appears before noise (raises Context Precision)",
            "Expand the golden dataset with the failing cases and re-run the "
            "benchmark after each fix to verify improvement",
        ]
